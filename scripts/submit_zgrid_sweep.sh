#!/bin/bash
# SLURM array job for the zgrid numerical-convergence sweep.
#
...
# chunks of 10. This sweep is about z-grid resolution, not SASE-shot
...
#
# Cost varies across this manifest: zgrid=50 is substantially slower than
# zgrid=5, though less steeply than the tgrid/xygrid sweeps. The
...
# generate_zgrid_sweep_configs.py's printed caution) and adjust.
#
# --array below is NOT authoritative: generate_zgrid_sweep_configs.py prints
...
#
# Before submitting:
#   1. python scripts/generate_zgrid_sweep_configs.py   (writes the manifest
...
#SBATCH --job-name=xlo-convergence-zgrid
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
MANIFEST=config/generated/convergence_zgrid/manifest.txt
if [[ ! -f "$MANIFEST" ]]; then
    echo "Missing $MANIFEST -- run scripts/generate_zgrid_sweep_configs.py first" >&2
    exit 1
fi
...
if [[ -z "$YAML" ]]; then
    echo "CONFIG_IDX $CONFIG_IDX has no entry in $MANIFEST (${#YAML_FILES[@]} configs) -- --array doesn't match the manifest; rerun scripts/generate_zgrid_sweep_configs.py and use the sbatch command it prints" >&2
    exit 1
fi
DATA_PATH=data/convergence_zgrid_${SLURM_ARRAY_JOB_ID}

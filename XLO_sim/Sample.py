...
class XLO_sample:
    ...
    def __init__(self, X, seed_field=None):
        ...
        if (seed_field is None):
            ...
            #self.seed_field = np.einsum('pstxy,qs->pqtxy', seed_field, np.linalg.inv(X.transform_matrix)) #???
...
#                 return rho_ijtxyz, rho_other_txyz, Omega_pstxyz

#             if X.enable_self_absorption == False:
#                 return rho_ijtxyz, Omega_pstxyz
    ...
    def configure(self, X):
        ...
        for j in range(0, X.zgrid):
            ...
            if (X.enable_pump_diffraction == False) and (j != 0):
                #j0_txy -= 0.5 * X.dz * j0_txy * (X.n * X.sigma_ground_pump * (rho_0_txyz[:, :, :, j - 1] + rho_0_txyz[:, :, :, j]) + X.n * X.sigma_compound_pump)
...
#        self.pump_itxyz = np.einsum('i, txyz->itxyz', X.ei_K, j_txyz * rho_0_txyz * X.sigma_1s / 2.0)
...
#                sqrtTrhoT_sxy = np.sqrt(np.einsum('isp,sjxy,jip -> pxy', X.Tijs_minus, rho_ijxy, X.Tijs_plus) + np.finfo(float).eps)
#                d_rho_it_noise = Model.MB_nlevel_noise(rho_ijxy, sqrtTrhoT_sxy, [X, it, iz])

                if X.is_use_stochastic == True:
              
                    E_sxy = np.einsum('isp,sjxy,jip -> pxy', X.Tijs_minus, rho_ijxy, X.Tijs_plus) + np.finfo(float).eps
                    G_sxy = np.einsum('isp,sjxy,jip -> pxy', X.Tijs_plus, rho_ijxy, X.Tijs_minus) 
                    d_rho_it_noise = Model.MB_nlevel_noise(rho_ijxy, E_sxy, G_sxy, [X, it, iz])

                
######################                
#                 if X.run_mode == 'consecutive':

#                     d_rho_it_reg = tools.RK45_step(Model.MB_nlevel_regular, rho_ijxy, it*X.dt, X.dt, [X, Omega_pstxy[:, :, it, :, :], it, iz, None, None, None, None])
#                     rho_ijxy += d_rho_it_reg 
#                     rho_ijtxyz[:, :, it, :, :, iz] = rho_ijxy
                             
#                     if (X.enable_self_absorption == True) and (iz != 0):

#                         d_rho_other_t = tools.RK45_step(Model.MB_other_regular, rho_other_xy, it * X.dt, X.dt, [X, Omega_pstxy[:, :, it, :, :], it, iz, None, None, None, None])
#                         rho_other_xy += d_rho_other_t
#                         rho_other_txyz[it, :, :, iz] = rho_other_xy
#                         kappa_Omega_psxyz = Model.absorption_Omega(rho_ijtxyz[:, :, it, :, :, iz-1:iz+1], rho_other_txyz[it, :, :, iz-1:iz+1], [X, it, iz])
...
#                     #!!!!!!!    
#                     #Omega_pstxy[:, :, it, :, :] +=  0.5 * X.dz * (Model.Omega_source_regular(rho_ijxy, [X, it, iz]) + Model.Omega_source_regular(rho_ijxy, [X, it, iz - 1])) + Model.Omega_source_noise(sqrtTrhoT_sxy, [X, it, iz])
#                     Omega_pstxy[:, :, it, :, :] +=  1.0 * X.dz * Model.Omega_source_regular(rho_ijxy, [X, it, iz]) 
                    
#                     if X.is_use_stochastic == True:
#                         Omega_pstxy[:, :, it, :, :] += Model.Omega_source_noise(E_sxy, G_sxy, [X, it, iz])
                    
#                         # According to Ito interpretation of stochastic differential equations, the noise from time "it" contributes to value at time "it+1"
#                         if (it != X.tgrid - 1):
#                             rho_ijxy += d_rho_it_noise 
#      #                       rho_ijtxyz[:, :, it + 1, :, :, iz] = rho_ijxy
...
#     def evaluate_rho_ij_with_pump_3D(self, X):
#             """
#             Perform calculation of the rho_ij with pump with no noise and field evolution
...
#     #                 E_sxy = np.einsum('isp,sjxy,jip -> pxy', X.Tijs_minus, rho_ijxy, X.Tijs_plus) + np.finfo(float).eps
#     #                 G_sxy = np.einsum('isp,sjxy,jip -> pxy', X.Tijs_plus, rho_ijxy, X.Tijs_minus) 
#                     d_rho_it_noise = 0.0 #Model.MB_nlevel_noise(rho_ijxy, E_sxy, G_sxy, [X, it, iz])
...
#                         if (X.enable_self_absorption == False) and (iz != 0):
#                             Omega_pstxy[:, :, it, :, :] = self.optics.Fresnel_propagator_no_absorption(X, Omega_pstxy[:, :, it, :, :], X.dz, iz*X.dz)   
#                         Omega_pstxy[:, :, it, :, :] +=  0.0 #1.0 * X.dz * Model.Omega_source_regular(rho_ijxy, [X, it, iz]) + Model.Omega_source_noise(E_sxy, G_sxy, [X, it, iz])
...
#                         Omega_pstxy[:, :, it, :, :] +=  0.0 #1.0 * X.dz * Model.Omega_source_regular(rho_ijtxyz[:, :, it, :, :, iz], [X, it, iz]) + Model.Omega_source_noise(E_sxy, G_sxy, [X, it, iz])

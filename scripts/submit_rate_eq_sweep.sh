...
# scripts/generate_rate_eq_sweep.sh -- RE points first, then the full-MB Cu-seed-SASE-double-satellite
# points, all on seeds 0..NREP-1, so the RE/MB ratio compares identical SASE realisations. The
# generator prints the matching --array range.
#
# NREP=200 on one 40-core node. Job 24549384 needed ~3-6 h per task at 8 h walltime and one task
# (RE 60 uJ) left no output at all, so 12 h leaves margin (incomplete tasks still leave a usable
# .partial.npz checkpoint every 40 reps).
#
...
#   3. sbatch --array=0-<N-1> scripts/submit_rate_eq_sweep.sh   (N printed by the generator)
